...
const Footer = () => {
  ...
  const handlerForm = (ev) => {
    ...
    const regexpForm = /(^[a-zA-Z0-9]{3,})@([a-zA-Z]{4,})\.([a-z]{2,3})$/;
    if (!valueForm.match(regexpForm)) errorMsg.innerHTML = '*The email it\'s not valid';
  ...
  return (
    <footer className="row p-5 m-0 d-flex flex-column flex-md-row flex-wrap justify-content-center justify-content-lg-around align-items-center footer">
      ...
      <div className="d-flex flex-column align-items-center align-items-lg-end justify-content-around col-12 col-lg-4 footer__copy">
        <form action="#" className="d-flex my-3 my-lg-none flex-row flex-nowrap" id="formFooter" onSubmit={(ev) => handlerForm(ev)}>
          <label className="d-flex flex-column" htmlFor="email" aria-label="Input the email">
            <input className="rounded-pill p-2 mr-3" type="email" id="email" placeholder="Updates in your inbox..." onChange={(ev) => handlerValue(ev)} />
            <span id="errorMsg" className="d-flex text-danger" />
          </label>
          <Button title="Go" typeBtn="submit" />
        </form>
        <small>
          Copyright 2020. All Rights Reserved
        </small>
